...
def normalize_headers(df: pd.DataFrame) -> pd.DataFrame:
    """Flatten MultiIndex, stringify, lowercase+strip headers, in-place; return df."""
...
def find_col(cols: Iterable[str], must_have: Optional[Iterable[str]] = None, any_of: Optional[Iterable[str]] = None) -> Optional[str]:
    names = list(cols)
    low = [str(c).lower() for c in names]
    must = [t.lower() for t in (must_have or [])]
    anyt = [t.lower() for t in (any_of or [])]
    for i, c in enumerate(low):
        if all(t in c for t in must) and (not anyt or any(t in c for t in anyt)):
            return names[i]
    return None


def pick_table_by_keywords(tables: List[pd.DataFrame], require: List[Dict]) -> Optional[pd.DataFrame]:
    for t in tables:
        df = t.copy()
        ...
        for rule in require:
            ...
            for col in df.columns:
                cl = str(col).lower()
                if all(m in cl for m in mh) and (not ao or any(a in cl for a in ao)):
                    hit = True; break
            if not hit:
                ok = False; break
        if ok:
...
_money_regex = re.compile(r"[0-9]+(?:,[0-9]{3})*(?:\.[0-9]+)?")

def to_money(s):
    s = str(s)
    m = _money_regex.findall(s)
...
def build_canonical_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    robust coercion. Also create safety aliases (gross/year/title/rank/peak) if missing.
    ...
    col_gross = find_col(df.columns, must_have=["gross"], any_of=["world","worldwide"])  
    col_year  = find_col(df.columns, any_of=["year","release","date"])  

    df["_rank"]  = pd.to_numeric(df[col_rank], errors="coerce") if col_rank else None
    df["_peak"]  = pd.to_numeric(df[col_peak], errors="coerce") if col_peak else None
    df["_title"] = df[col_title].astype(str) if col_title else ""
    df["_gross"] = df[col_gross].map(to_money) if col_gross else None
    if col_year:
        df["_year"] = df[col_year].map(to_year)
    else:
        df["_year"] = df["_title"].map(to_year)

    # Safety aliases to avoid KeyErrors if code references simple names later
